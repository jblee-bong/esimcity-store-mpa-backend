...
@Component
public class OriginWorldMoveUtil {
    ...
    @Value("${api.worldmove.merchantId}")
    public void setMerchantId(String merchantId) {
        this.merchantId = merchantId;
    }

    public static String deptId;
    @Value("${api.worldmove.deptId}")
    public void setDeptId(String deptId) {
        this.deptId = deptId;
    }

    public static String token;
    @Value("${api.worldmove.token}")
    ...
    public static void  contextLoads4(String rcode, String iccid, String productId, Model model) throws Exception {
        ...
        if(esimMap.get("code").toString().equals("0")){
            ...
            String str = sdf.format(new Date(Long.parseLong(esimMap.get("useEDate").toString())));

            Date expiredAt =  sdf.parse(str);
            Date now = new Date();


            model.addAttribute("end",now.after(expiredAt));

            model.addAttribute("expiredAt",str);//종료일이 매일 갱신 시간은 존재하지 않음. 이걸로 최종 종료일 해야함
